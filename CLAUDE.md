...
2. [UI要件書](documents/2_UI要件書.md) - UI/UX要件、カラーパレット、画面構成
3. [機能設計書](documents/3_機能設計書.md) - 機能仕様、操作フロー
4. [アーキテクチャ設計書](documents/4_アーキテクチャ設計書.md) - 技術スタック、システム構成
5. [データベース設計書](documents/5_データベース設計書.md) - テーブル定義、RLSポリシー
6. [利用規約](documents/6_利用規約.md) - サービス利用規約
7. [プライバシーポリシー](documents/7_プライバシーポリシー.md) - 個人情報保護方針
...
**1. medal_mst_seasonsテーブル** (シーズンマスタ):
- `season_no` (integer, 主キー, GENERATED BY DEFAULT AS IDENTITY) - シーズン番号
- `year` (integer, NOT NULL) - 年（2025年以降）
- `season` (season_enum, NOT NULL) - 四季（春/夏/秋/冬）
- `display_name` (varchar(20), NOT NULL) - 表示名（例：2025/秋）
- `is_current` (boolean, NOT NULL, DEFAULT false) - 現在シーズンフラグ
- `created_at` (timestamptz, NOT NULL) - 作成日時
- UNIQUE制約: (year, season) - 年と四季の組み合わせで一意

**2. medal_medalsテーブル** (メダル位置情報):
- `medal_no` (bigint, 主キー, GENERATED BY DEFAULT AS IDENTITY) - メダル番号（連番）
- `user_id` (uuid, auth.usersへの外部キー)
- `season_no` (integer, medal_mst_seasonsへの外部キー)
- `latitude` (decimal(10,8), NOT NULL) - 緯度
- `longitude` (decimal(11,8), NOT NULL) - 経度
- `created_at` (timestamptz, NOT NULL) - 作成日時
- `updated_at` (timestamptz, NOT NULL) - 更新日時

**3. medal_reportsテーブル** (誤メダル通報):
- `id` (bigint, 主キー, GENERATED BY DEFAULT AS IDENTITY) - 通報ID（連番）
- `medal_no` (bigint, medal_medalsへの外部キー)
- `reporter_user_id` (uuid, auth.usersへの外部キー)
- `created_at` (timestamptz, NOT NULL) - 通報日時
- UNIQUE制約: (medal_no, reporter_user_id) - 重複通報防止

**Row Level Security (RLS) ポリシー**:
- **medal_medals SELECT**: 全員が閲覧可能（`USING (true)`）
- **medal_medals INSERT**: 認証済みユーザーのみ、`auth.uid() = user_id`で検証
- **medal_medals DELETE**: 認証済みユーザーは自分のメダルのみ削除可能
...
### メダル登録ロジック（F002）
1. ユーザーがマップを長押し（または「登録」ボタンをタップ）
2. アプリが現在のGPS座標を取得
3. **クライアントから直接INSERT**:
   ```typescript
   await supabase.from('medal_medals').insert([{
     user_id: supabase.auth.user().id,
     season_no: currentSeasonNo,
...
### メダル表示ロジック（F003）
- **探検モード**: 未獲得メダルは金色、獲得済みメダルはグレーで表示
- **登録＆獲得モード**: 自分のメダルは金色、他人のメダルはグレーで表示
...
6. 通報データは蓄積され、運営が確認可能
7. メダル削除・ユーザーBAN判断は運営が手動で実施

### パフォーマンス最適化
- **シーズンインデックス**: 現在シーズンのメダルのみを効率的に取得
- **地理座標インデックス**: 範囲検索を高速化
- **複合インデックス**: season_no + latitude + longitude で最適化

## 開発ガイドライン

### Supabaseクライアント利用パターン
- **認証が必要な操作の前には必ず`supabase.auth.user()`をチェック**
- **現在シーズンのメダルのみを取得**: `.eq('season_no', currentSeasonNo)`でフィルタリング
...
### データベースセットアップ
1. Supabaseプロジェクトを作成
2. [documents/5_データベース設計書.md](documents/5_データベース設計書.md)のSQLを実行:
   - medal_mst_seasonsテーブル作成
   - medal_medalsテーブル作成
   - medal_reportsテーブル作成
   - medal_user_acquisitionsテーブル作成
   - インデックス作成
   - RLSポリシー設定
...
2. **[UI要件書](documents/2_UI要件書.md)** - UI/UX要件、カラーパレット、画面構成
3. **[機能設計書](documents/3_機能設計書.md)** - 機能仕様、機能フロー、用語集
4. **[アーキテクチャ設計書](documents/4_アーキテクチャ設計書.md)** - 技術スタック、システム構成図、B案採用理由
5. **[データベース設計書](documents/5_データベース設計書.md)** - テーブル定義、RLSポリシー、データアクセスパターン
6. **[利用規約](documents/6_利用規約.md)** - サービス利用規約
7. **[プライバシーポリシー](documents/7_プライバシーポリシー.md)** - 個人情報保護方針
...
- シンプルで直感的な長押し登録
- **探検モード**: 未獲得メダルは金色、獲得済みメダルはグレー
- **登録＆獲得モード**: 自分のメダルは金色、他人のメダルはグレー
