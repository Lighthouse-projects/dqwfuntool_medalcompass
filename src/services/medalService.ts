 ...
 * @param radiusMeters 半径（メートル）デフォルト5m
 * @returns 周辺のメダル配列
 */
export async function getNearbyMedals(
  latitude: number,
  longitude: number,
  ...
  try {
    // 範囲の矩形を計算
    const bounds = calculateBoundingBox(latitude, longitude, radiusMeters);
    ...
    if (error) {
      console.error('Get nearby medals error:', error);
      throw new Error('周辺のメダル取得に失敗しました');
    }
...
/**
 * 5メートル以内に重複するメダルがあるかチェック
 * @param latitude 緯度
 * @param longitude 経度
 * @returns 重複があればtrue
 */
export async function checkDuplicateWithin5Meters(
  latitude: number,
  longitude: number
): Promise<boolean> {
  try {
    // 周辺5メートル以内のメダルを取得
    const nearbyMedals = await getNearbyMedals(latitude, longitude, 5);

    // Haversine距離計算で厳密にチェック
