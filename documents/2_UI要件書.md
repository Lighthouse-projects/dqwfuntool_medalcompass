...
- サポートセクション
  - お知らせリンク（タップでお知らせ一覧画面へ遷移）
    - 緊急・警告のお知らせがある場合、バッジ表示（例：🔴 2）
    - バッジには緊急＋警告の合計件数を表示
    - お知らせがない場合はバッジ非表示
...
- お知らせバッジ件数: medal_announcementsテーブルから緊急・警告の有効期間内お知らせ件数を取得
- 初回マウント時に非同期で読み込み
...
**表示条件:**
- **緊急（emergency）のお知らせがある場合のみ表示**
- 情報（info）や警告（warning）のお知らせではバナー表示しない
- 有効期間内の緊急お知らせがない場合はバナー非表示

**動作要件:**
- アプリ起動時、有効期間内の緊急お知らせがあれば表示
- バナータップでお知らせ一覧画面へ遷移
- ×ボタンタップでバナーを閉じる
- アプリ再起動時に再表示される（既読管理なし）
- 複数の緊急お知らせがある場合、最新のものを表示
