...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | 通報ID（連番） |
...
COMMENT ON COLUMN medals.medal_no IS 'メダル番号（連番、何個登録されたかがわかる）';
...
COMMENT ON COLUMN medals.is_deleted IS '削除フラグ（5通報以上でtrue、デフォルトfalse）';
COMMENT ON COLUMN medals.deleted_at IS '削除日時（is_deleted=trueになった日時）';
...
COMMENT ON COLUMN medal_reports.id IS '通報ID（連番）';
COMMENT ON COLUMN medal_reports.medal_no IS '通報対象のメダル番号';
...
**用途**: マップ表示範囲内の有効なメダルを取得

**実装例（TypeScript）**:
```typescript
const { data: medals } = await supabase
  .from('medals')
  .select('medal_no, user_id, latitude, longitude, created_at')
  .eq('is_deleted', false)  // 削除フラグfalseのみ取得
...
**パフォーマンス**:
- `idx_medals_is_deleted`インデックスで削除済みメダルを高速除外
- `idx_medals_location`インデックスで範囲検索を高速化
...
### 6. メダルの通報数取得とis_deleted更新（クライアントから）

**用途**: 特定のメダルの通報数を取得し、5通報以上なら削除フラグを立てる
...
const { data: reports, count } = await supabase
  ...
  .eq('medal_no', targetMedalNo);

// 2. 5通報以上ならis_deletedを更新（管理者またはバッチ処理で実施）
if (count >= 5) {
  await supabase
    .from('medals')
    .update({
...
**実行場所**: クライアント（React Native）またはバックグラウンド処理
**ロジック**:
- 通報INSERT後に通報数をカウント
- 5通報に達したらis_deletedをtrueに更新（deleted_atに現在日時を記録）
- 以降はis_deleted=falseフィルタで自動的に非表示

**注意**:
- medals テーブルのUPDATE権限が必要（別途RLSポリシー追加が必要な場合がある）
- または管理者がSupabaseダッシュボードから手動更新

---

### 7. ユーザーの通報受信数取得とBAN処理（管理者用）

**用途**: 特定のユーザーが何回通報を受けたかを取得し、10通報以上なら全メダルを削除

**実装例（TypeScript）**:
```typescript
// 1. ユーザーが登録した全メダルの通報数を集計
...
// 2. 10通報以上ならユーザーの全メダルをis_deleted=trueに更新
if (count >= 10) {
  await supabase
    .from('medals')
    .update({
...
- 10回以上ならそのユーザーの全メダルのis_deletedをtrueに更新
- 以降は全メダルが自動的に非表示になる
...
**用途**: 不正ユーザーのメダルを一括で無効化または物理削除

**論理削除（推奨）**:
```sql
-- is_deletedフラグで論理削除（データは残る）
UPDATE medals
...
**用途**: 不正データを登録したユーザーの全メダルを無効化または削除
**推奨**: 論理削除により監査証跡を保持
