...
- **Edge Functions**: Supabase Edge Functions（アカウント削除のみ使用）
- **ストレージ**: 不要
- **通知・音響**: 不要
- **ナビゲーション**: 不要
...
6. **アカウント削除**:
   - クライアントからEdge Function（delete-account）を呼び出し
   - Edge Function内でパスワード再確認後、管理者権限でユーザー削除
   - ON DELETE CASCADEで関連データが自動削除
...
- **Supabase Edge Functions**: Deno runtime、アカウント削除時の管理者権限操作に使用。無料枠で十分対応可能
