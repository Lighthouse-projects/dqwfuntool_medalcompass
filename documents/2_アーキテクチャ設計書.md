...
graph TB
    subgraph "Client Layer - モバイルアプリ (iOS/Android)"
        A[React Native + Expo]
        B[Expo Router<br/>画面遷移]
        C[React Context<br/>AuthContext / UnreadContext]
        D[Expo SecureStore<br/>トークン保存]
        E[Expo Local Authentication<br/>生体認証]
    ...
    subgraph "Supabase Platform - BaaS"
        ...
        J[Supabase Realtime<br/>WebSocket]
    ...
    subgraph "Infrastructure - インフラ"
        O[Google Cloud Storage<br/>大容量ストレージ]
        P[Cloud CDN<br/>コンテンツ配信]
    end

    subgraph "Device Layer - デバイス"
        R[iOS Keychain<br/>Android Keystore]
    ...
    classDef infraStyle fill:#f8d7da,stroke:#dc3545,stroke-width:2px
    classDef deviceStyle fill:#e2e3e5,stroke:#6c757d,stroke-width:2px

    class A,B,C,D,E,F clientStyle
    class G,H,I,J,K supabaseStyle
    class L,M,N edgeFuncStyle
    class O,P,Q infraStyle
    class R,S deviceStyle
```
...
- Expo Routerでファイルベースルーティング
- React Contextで認証・通知状態を管理
- Expo SecureStoreで安全なトークン保存
- 生体認証でユーザー体験向上
...
**Infrastructure (インフラ)**
- Google Cloud Storageで大容量データ保存
- Cloud CDNで高速コンテンツ配信

**Device Layer (デバイス)**
- iOS Keychain / Android Keystoreでトークン暗号化
...
3. **リアルタイム**: WebSocketでデータ変更を即座に全クライアントへ配信
4. **認証**: JWT トークンで全リクエストを検証
